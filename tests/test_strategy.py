...
def test_entry_uses_btc_ma20_or_rsi40_market_filter():
    ...
    assert ok
    assert "BB_REENTRY_RSI30_RECLAIM" in reason

    ok, reason = entry_signal(_entry_row(close=99.0, ma20=100.0, rsi=31.0), previous, ticker="KRW-BTC", config=StrategyConfig())
    assert not ok
    assert "MARKET=N" in reason

    ok, reason = entry_signal(
        _entry_row(close=99.0, low=97.0, bb_lower=98.0, ma20=100.0, rsi=41.0),
        previous,
        ticker="KRW-BTC",
...
def test_entry_requires_btc_market_filter_for_alts():
    ...
    ok, reason = entry_signal(
        ...
        previous,
        ticker="KRW-ETH",
        market_current=_entry_row(close=90.0, ma20=100.0, rsi=41.0),
    )
    assert ok
...
def test_entry_requires_current_volume_to_exceed_previous_five_average():
    ok, reason = entry_signal(
        _entry_row(volume=100.0, prev5_volume_avg=100.0),
        _entry_row(rsi=29.0),
        ticker="KRW-BTC",
        config=StrategyConfig(),
    )
...
def test_entry_blocks_overbought_and_ma20_disparity():
    ...
    ok, reason = entry_signal(_entry_row(rsi=61.0), previous, ticker="KRW-BTC")
    assert not ok
...
def test_exit_signal_conditions():
    base = _entry_row(close=105.0, low=105.0, rsi=50.0, bb_upper=120.0, ma120d=90.0)

    exit_price, reason, armed, highest = exit_signal(_series_with(base, close=102.0), entry_price=100.0, current_price=102.0)
    assert exit_price is None
    assert "HOLD" in reason
    assert armed
    assert highest == 102.0

    exit_price, reason, armed, highest = exit_signal(
    ...
    exit_price, reason, _, _ = exit_signal(
        _series_with(base, low=92.0, atr14=4.0),
        entry_price=100.0,
        current_price=96.0,
        use_ohlc=True,
    )
    ...
    exit_price, reason, _, _ = exit_signal(_series_with(base, close=89.0), entry_price=80.0, current_price=89.0)
    assert exit_price == 89.0
    assert reason == "MA120_BREAKDOWN"


def test_dead_cross_exit_has_top_priority():
    current = _entry_row(close=96.0, low=92.0, ma5=99.0, ma20=100.0, atr14=4.0)
...
def test_time_cut_after_48_bars_when_profit_is_too_small():
    current = _entry_row(close=100.4, high=100.4, low=100.4, ma120d=90.0)
    ...
    assert reason.startswith("TIME_CUT")


def test_step_up_trailing_take_profit_after_five_percent_profit():
    base = _entry_row(close=110.0, low=108.0, high=112.0, bb_upper=120.0, rsi=50.0)
...
def test_base_trailing_take_profit_before_five_percent_profit():
    base = _entry_row(close=103.0, low=102.4, high=104.0, bb_upper=120.0, rsi=50.0)
...
def test_adaptive_stop_and_risk_position_size():
    ...
    assert adaptive_stop_price(100.0, row) == 94.0
    assert adaptive_stop_price(100.0, _entry_row(atr14=10.0)) == 93.0
...
def test_check_signal_compatibility_wrappers():
    ...
    sell_ok, sell_reason, armed, highest = check_sell_signal(
        100.0,
        108.0,
        row=_entry_row(high=112.0, low=108.0),
        take_profit_armed=True,
        highest_price=112.0,
    )
    assert sell_ok
